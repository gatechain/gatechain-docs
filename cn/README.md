...
<!--GateChain 针对个人，专业用户和企业用户提供丰富的钱包应用。 测试网阶段提供基于Windows和Mac OS平台的桌面版钱包，专业用户也可以通过RPC方式访问和测试。主网阶段还将提供命令行钱包，以及基于Android和iOS的手机版钱包，Web钱包，多签钱包，硬件钱包等多种针对个人和企业的生态工具。-->
...
- **全节点和本地测试网**：每个用户都可以自由搭建并运行一个完整的节点并运行自己的测试网络。全节点具有GateChain所有功能。详细安装部署步骤介绍请参阅“[安装部署](./gatechain-build.md)”。
  ...
  - **RPC API**： GateChain节点提供高级RPC API接口服务，相关API信息请参阅“[RPC API 接口列表](./gatechain-rpc.md)”。
