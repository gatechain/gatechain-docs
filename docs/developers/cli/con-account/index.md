...
### 修改共识账户最大手续费率
```bash
gatecli con-account max-rate -- commission-max-rate [最大手续费率] --commission-max-change-rate [每天最大手续费率变化] --from [发送者账户] --fees [交易手续费] --chain-id [链ID]
```
::: details 请求示例:
...
### <span id="查询共识账户列表">查询共识账户列表 [API](../../api/con-account/index.md#查询共识账户列表-命令行)</span>
```bash
